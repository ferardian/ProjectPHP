<?php 
	class Report extends CI_Controller{
		...
		function __construct(){
			parent:: __construct();
			$this->load->model(array('report_mod','rekap_mod'));
			if (!is_login()) {
		...
		function index(){ 
			...
			$this->template->load('template_admin','v_report');
			// $this->load->view('template_admin',$data);
		}

		function index_admin(){
			$this->template->load('template_super_admin','v_report');
		}
		...
		function report_inmut(){ 
			...
			$this->template->load('template_admin','v_report_inmut',$data);
			// $this->load->view('template_admin',$data);
		}

		function report_inmut_admin(){
			$data['data_ruang'] = $this->report_mod->getruang()->result();
			$data['data_indikator'] = $this->report_mod->getindikator_ruang()->result();
			$this->template->load('template_super_admin','v_report_inmut_semua',$data);
		}
		
		function report_ikp(){ 
			
			$this->template->load('template_admin','v_report_ikp');
		}

		function report_ikp_admin(){
			$this->template->load('template_super_admin','v_report_ikp');
		}

		public function daftar_analisa(){ 
		
		$session_id = $this->session->userdata('kd_ruang');
		$user = $this->rekap_mod->getid($session_id)->row_array();
		$ruangan = $this->report_mod->daftar_analisa($user['kd_ruang'])->result();
		$data['record']=$ruangan;
		$this->template->load('template_admin','indikator/v_daftar_analisa',$data);
		}

		function daftar_analisa_semua(){
			$data['record']=$this->report_mod->daftar_analisa_semua()->result();
			$this->template->load('template_super_admin','indikator/v_daftar_analisa_semua',$data);
		}
		...
		function hapus_analisa(){
			$this->db->where('kd_analisa', $this->uri->segment(3));
			$this->db->delete('tb_analisa');
		...
		function post_inmut(){
			...
			return $this->template->load('template_admin','v_rekap_inmut',$query);
				...
				$ruang = $this->input->post('kd_ruang');
				$inmut = $this->input->post('kd_inmut');
				$query['record']= $this->db->query('SELECT * FROM tb_rekap_inmut join tb_ruangan on tb_rekap_inmut.kd_ruang=tb_ruangan.kd_ruang join tb_inmut on tb_rekap_inmut.kd_inmut=tb_inmut.kd_inmut where tanggal between "'.$tanggal_awal.'" and "'.$tanggal_akhir.'" and tb_rekap_inmut.kd_inmut="'.$inmut.'" and tb_rekap_inmut.kd_ruang="'.$ruang.'"')->result();
				//$query['tgl_awal']=$tanggal_awal;
				//$query['tgl_akhir']=$tanggal_akhir;
				$query['ruangan']=$ruang;
				$query['nama_indikator']=$inmut;
				$query['kd_inmut']=$inmut;
				$query['tanggal']=$tanggal_awal;
				$html = $this->load->view('v_detail_indikator',$query, true);
				$pdfFilePath='indikator_detail.pdf';
	...
	function post_inmut_semua(){
			if (isset($_POST['submit'])){
				...
				$query['tgl_awal']=$tanggal_awal;
				$query['tgl_akhir']=$tanggal_akhir;
			return $this->template->load('template_super_admin','v_rekap_inmut_semua',$query);
